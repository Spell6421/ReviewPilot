...
| **No immediate retry / backoff on failed sends** | `lib/send-message.ts`, `lib/automations/run.ts` | Low | *Partially paid down:* the automation `find*` dedup now excludes `failed` rows, so a failed automated send self-heals — it's re-found and re-sent on the next nightly run instead of being permanently masked (each automation's window bounds the retries). Remaining gap: no *immediate* in-call retry/backoff, and the manual UI send path is still single-shot (a failed manual send just surfaces an error for the owner to re-click). |
